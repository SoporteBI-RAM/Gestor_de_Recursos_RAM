...
function doGet(e) {
  ...
  const bustCache = e.parameter._t; // Cache buster timestamp
  ...
  try {
    ...
    if (action === 'read' && sheetName) {
      ...
      // Usar cache de 60 segundos para acelerar (EXCEPTO si hay cache buster)
      const cache = CacheService.getScriptCache();
      const cacheKey = 'sheet_' + sheetName;
      const cached = !bustCache ? cache.get(cacheKey) : null; // Si hay bustCache, ignorar cache
